...
In the fields pane, hovering a row highlights its bytes in the hex view and clicking a byte selects the field that covers it (for boxes with a field map: `ftyp`/`styp`, `mvhd`, `tkhd`, `mdhd`, `hdlr`, `trex`, `elst`, `mfhd`, `tfhd`, `tfdt`, `trun`, `sidx`, `emsg`). Besides the library's readers, the panel decodes `senc` (IV size inferred), `saiz`, `saio`, `avcC`, `hvcC`, `av1C`, `vpcC`, `esds` (incl. AudioSpecificConfig) and `dOps`. `pssh` boxes are named by DRM system (Widevine, PlayReady, FairPlay, ClearKey, Marlin) and list their key IDs (v1 list, Widevine and PlayReady payloads); `tenc` shows the default KID as a UUID. With an init segment selected, the tree header offers a **diff with…** select to compare its decoded fields with another init.
